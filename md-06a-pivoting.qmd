...
2.  What's the `tidyverse` Package? Describe in maximum two sentences below.\
    Tidyverse is a package that is useful in "everyday data analyses" ([https://www.tidyverse.org/packages/](#0)). It includes the so far for us interesting packages ggplot2, dplyr, tidyr, readr.
...
1.  Starting with the `waste_data_long_mean` object, execute the code and observe the output in the Console. Would you publish this table in this format in a report? Rather not.
...
2.  Execute the code and observe the output in the Console. Would you publish this table in a report in this format? Still no. I mean the format is alright, the layout could be better...
...
waste_data_long_mean |> 
  ...
  scale_fill_brewer(palette = "RdBu") +   #https://www.perplexity.ai/search/how-do-you-FXxstEENSsCEf4KwwD_5rA
  theme_classic()
```
